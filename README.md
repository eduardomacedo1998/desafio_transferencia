...
#### **Inventory** (Estoque)
Esta deve ser a tabela pivô que liga Produtos a Armazéns, rastreando a quantidade.

| Campo | Tipo | Descrição |
|-------|------|-----------|
| `warehouse_id` | foreign key | Referência ao armazém |
| `product_id` | foreign key | Referência ao produto |
| `quantity` | integer | Quantidade (padrão 0) |
...
| `source_warehouse_id` | foreign key | Armazém de origem |
| `destination_warehouse_id` | foreign key | Armazém de destino |
| `quantity` | integer | Quantidade transferida |
| `user_id` | foreign key (opcional) | Usuário responsável (para auditoria) |
